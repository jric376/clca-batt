...
get_bsrn.cove = function(overwrite = FALSE) {
  ...
  bsrn_df.cove.days = bsrn_df.cove.days %>%
                  mutate(ghi = case_when(bsrn_df.cove.days$ghi < 3 & bsrn_df.cove.days$ghi > -3 ~ 0,
                                          bsrn_df.cove.days$ghi != -999 ~ bsrn_df.cove.days$ghi)) %>%
                  fill(ghi, .direction = "up") %>%
                  fill(ghi) %>%
                  mutate(index = strftime(date_time, format = "%Y-%m-%d"),
                         day_ind = strftime(date_time, format = "%j"))
  bsrn_df.cove.days = inner_join(bsrn_df.cove.days, data.frame(index = sample_dt, stringsAsFactors = FALSE))
  ...
  if(overwrite) {  
    ...
    if(station == "larc"){
      bsrn_df = get_bsrn.larc() %>%
        select(-clr_ghi,-(kt.sum:weather)) %>%
        mutate_if(is.integer, as.numeric)
      bsrn_df = left_join(bsrn_df, bsrn_df.clearsky)
    }
