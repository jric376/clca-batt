...
# Takes in the following load_profile objects:
# grid load (MW), bldg load (kW), PV load (kW)

# It also takes in a battery bank object
# and a demand response target (dmd_targ)
...
# dataframe for up to 1-yr of simulation.
 ...
 public = list(
   ...
   operate = function(timestep = NULL, bldg_kw = NULL,
     ...
     if (bldg_kw > private$dmd_targ) {
       ...
       if (pv_kw > bldg_kw) {
         # if there is more solar production
         # than bldg load:
         # - satisfy all bldg load
         # - charge battery as much as poss.
         # - curtail the rest
         ...
         # if there is some solar:
         # - satisfy max bldg load
         # - (dis)charge battery as needed
         # - curtail the rest
         ...
         # if there is no solar production:
         # - set bldg load to target
         # - (dis)charge battery as needed
         # - unmet load if not enough batt avail.
       ...
       if (pv_kw > 0) {
         # if there is some solar:
         # - charge battery as much as poss.
         # - reduce bldg load w remaining solar
         ...
         if ((hr >= 1 | hr <= 5) & private$batt$soc <= 0.5) {
             max_draw = (private$dmd_targ - bldg_kw)*0.8
     ...
     if(private$metadata[["ctrl_id"]] == "batt_sizer") {
       ...
       grid_plc2erta <- (grid_kw*0.001)*as.numeric(private$metadata[["time_int"]])* 
                            private$disp$get_emish(iso_mw)
       private$disp$stochastize_costs(FALSE)
     }
   ...
   traverse_ts = function(n = "full", log = FALSE, save_df) {
     ...
     if(is.numeric(n)) {
       ...
       steps = c(1, length(bldg_kw) - 1)
     }
     
     sim_df <- bind_rows(lapply(steps[1]:steps[2], function(i) {
       if(log) {
